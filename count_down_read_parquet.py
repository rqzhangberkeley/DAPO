...
if __name__ == "__main__":
    ...
    try:
        train_df = read_parquet_file(os.path.join(data_dir, "count_down_mix_easy_train.parquet"))
        print("\nTrain data sample:")
    ...
    # check the distribution of the dataset difficulty for the mixed dataset
    train_df = read_parquet_file(os.path.join(data_dir, "count_down_mix_easy_train.parquet"))
    test_df = read_parquet_file(os.path.join(data_dir, "count_down_mix_easy_test.parquet"))

    level_distribution = {}
    for i in range(len(train_df)):
        level = train_df.iloc[i]['extra_info']['level']
    ...
    for i in range(len(test_df)):
        level = test_df.iloc[i]['extra_info']['level']
        if level not in level_distribution:
            level_distribution[level] = 0
        level_distribution[level] += 1
    print(level_distribution)
