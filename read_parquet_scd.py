...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', default='simple_count_down_base')
    ...
    try:
        train_df = read_parquet_file(os.path.join(data_dir, "count_down_mix_middle_train.parquet"))
        # print("\nTrain data sample:")
        ...
        nums_1 = [988, 118, 153, 288, 914, 299, 847]
        nums_2 = [830, 583, 551, 700, 571, 542]

        for i, row in train_df.iterrows():
            if list(row['reward_model']['ground_truth']['numbers']) == nums_1:
                print(row['prompt'], row['extra_info'], row['reward_model']['ground_truth'])
            elif list(row['reward_model']['ground_truth']['numbers']) == nums_2:
                print(row['prompt'], row['extra_info'], row['reward_model']['ground_truth'])
    except Exception as e:
        print(f"Error processing train data: {e}")

    # try:
    #     test_df = read_parquet_file(os.path.join(data_dir, "count_down_mix_middle_test.parquet"))
    #     print("\nTest data sample:")
