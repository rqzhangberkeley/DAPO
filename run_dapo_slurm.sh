#!/bin/bash
#SBATCH --job-name=1.5B-DAPO-noCL       # Job name
#SBATCH --output=./logs/DAPO-NOCL-1.5B_%j.out  # Output file (%j will be replaced by job ID)
#SBATCH --error=./logs/DAPO-NOCL-1.5B_%j.err   # Error file
...
export PYTORCH_CUDA_ALLOC_CONF="max_split_size_mb:128,expandable_segments:True" # RZ: When using DeltaAI machine, sometimes it will crash due to CUDA out of memory. This is a workaround. You can reduce the `max_split_size_mb` to 32 or 64 if you still encounter the issue.
...
project_name='DAPO'
exp_name='1.5B-vanilla-DAPO-max_split_size_mb:128'
