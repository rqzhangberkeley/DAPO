#!/bin/bash
#SBATCH --job-name=FAST-GRPO-7B-DAPO-N4+20-offload       # Job name
#SBATCH --output=./logs/FAST-GRPO-7B-DAPO-N4+20-offload_%j.out  # Output file (%j will be replaced by job ID)
#SBATCH --error=./logs/FAST-GRPO-7B-DAPO-N4+20-offload_%j.err   # Error file
...
project_name='DAPO'
exp_name='7B-Math-FAST-GRPO-dataDAPO-N4+20-offload'
...
CKPT_PATH=${CKPT_PATH:-"/work/hdd/beok/rzhang15/checkpoints/GRPO/${exp_name}/$(date +%Y%m%d_%H%M%S)"}
mkdir -p ${CKPT_PATH}
