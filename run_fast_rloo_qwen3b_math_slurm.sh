#!/bin/bash
#SBATCH --job-name=FAST-RLOO-Qwen-3B-DeepMath-N12+12-offload-Llama-bsz4-16-1e-6-test       # Job name
#SBATCH --output=./logs/FAST-RLOO-Qwen-3B-DeepMath-N12+12-offload-Llama-bsz4-16-1e-6-test_%j.out  # Output file (%j will be replaced by job ID)
#SBATCH --error=./logs/FAST-RLOO-Qwen-3B-DeepMath-N12+12-offload-Llama-bsz4-16-1e-6-test_%j.err   # Error file
#SBATCH --nodes=1                 # Number of nodes
#SBATCH --ntasks-per-node=1       # Number of tasks per node
#SBATCH --cpus-per-task=32         # Number of CPU cores per task
#SBATCH --gpus-per-node=4              # Number of GPUs (4 GPUs per node)
#SBATCH --mem=500G                # Memory per node
#SBATCH --time=3:59:59           # Time limit (24 hours)
...
project_name='DAPO'
exp_name='3B-DeepMath-FAST-RLOO-N12+12-offload-Qwen-bsz4-16-lr1e-6-test'
...
mkdir -p ${CKPT_PATH}
TRAIN_FILE=${TRAIN_FILE:-"./data/DeepMath-Qwen-base/train.parquet"} # RZ: New experiments. We use Math500.
...
PYTHONUNBUFFERED=1 python3 -m recipe.dapo.src.main_fast_dapo \
    ...
    data.val_files=[\"./data/AIME2024-dup16-Qwen-base/train.parquet\",\"./data/AIME2025-dup16-Qwen-base/train.parquet\",\"./data/math500-Qwen-base/test.parquet\",\"./data/AMC23-dup4-Qwen-base/train.parquet\",\"./data/gsm8k-Qwen-base/test.parquet\"] \
