#!/bin/bash
#SBATCH --job-name=FAST-RLOO-3B-SCD-MixedK-N12+12-offload-B8-lr1e-6-middle      # Job name
#SBATCH --output=./logs/FAST-RLOO-3B-SCD-MixedK-N12+12-offload-B8-lr1e-6-middle_%j.out  # Output file (%j will be replaced by job ID)
#SBATCH --error=./logs/FAST-RLOO-3B-SCD-MixedK-N12+12-offload-B8-lr1e-6-middle_%j.err   # Error file
#SBATCH --nodes=1                 # Number of nodes
#SBATCH --ntasks-per-node=1       # Number of tasks per node
#SBATCH --cpus-per-task=32         # Number of CPU cores per task
#SBATCH --gpus-per-node=4              # Number of GPUs (4 GPUs per node)
#SBATCH --mem=450G                # Memory per node
#SBATCH --time=00:20:00           # Time limit (24 hours)
...
project_name='DAPO'
exp_name='3B-SCD-MixedK-N12+12-offload-B8-lr1e-6-middle'
...
mkdir -p ${CKPT_PATH}
TRAIN_FILE=${TRAIN_FILE:-"./data/simple_count_down_base/count_down_mix_middle_train.parquet"} # RZ: New experiments. We use simple countdown.
...
#     -- 
PYTHONUNBUFFERED=1 python3 -m recipe.dapo.src.fast_dapo_mixedk_main \
    data.train_files="${TRAIN_FILE}" \
    data.val_files=[\"./data/simple_count_down_base/count_down_mix_middle_test.parquet\"] \
