#!/bin/bash
#SBATCH --job-name=RLOO-3B-SCD-N24-offload-B8-lr1e-6-middle-correct       # Job name
#SBATCH --output=./logs/RLOO-3B-SCD-N24-offload-B8-lr1e-6-middle-correct_%j.out  # Output file (%j will be replaced by job ID)
#SBATCH --error=./logs/RLOO-3B-SCD-N24-offload-B8-lr1e-6-middle-correct_%j.err   # Error file
#SBATCH --nodes=1                 # Number of nodes
#SBATCH --ntasks-per-node=1       # Number of tasks per node
#SBATCH --cpus-per-task=32         # Number of CPU cores per task
#SBATCH --gpus-per-node=4              # Number of GPUs (4 GPUs per node)
#SBATCH --mem=450G                # Memory per node
#SBATCH --time=9:59:59           # Time limit (24 hours)
...
project_name='DAPO'
exp_name='3B-SCD-RLOO-N24-offload-B8-lr1e-6-middle-correct'
